...
const game = {
    ...
    stage: 1,
    shotType: 'normal',
    stats: { atk: 5, atkLevel: 1, critRate: 0.05, critLevel: 1 },
    ...
    spawnEnemy: () => {
        ...
        game.enemy.shake = 0;
        const types = [{name: "Slime", color: "#2ecc71"}, {name: "PurpleSlime", color: "#9b59b6"}, {name: "BlueSlime", color: "#0b36d2ff"}, {name: "WhiteSlime", color: "#e3dbdbff"}, {name: "RedSlime", color: "#e74c3c"}];
    ...
    attack: () => {
        ...
        if (game.user_token < cost) {
            if(game.autoActive) game.toggleAuto();
    ...
    onWin: () => {
        const baseReward = Math.floor(game.enemy.maxHp / 5); 
        ...
        if (game.autoActive) {
            ...
            api.saveGame();
            setTimeout(() => { game.stage++; game.spawnEnemy(); game.updateUI(); }, 600);
        } else {
    ...
    updateUI: () => {
        ui.user_token.innerText = game.user_token;
        ui.stage.innerText = "Lv." + game.stage;
        ui.hp.innerText = Math.floor(game.enemy.currentHp) + " / " + game.enemy.maxHp;
...
const doubleUp = {
    ...
    nextRound: () => {
        ...
        document.getElementById('cardDealer').innerText = doubleUp.toCardStr(doubleUp.dealerVal);
        document.getElementById('cardDealer').style.color = (doubleUp.dealerVal > 10 || doubleUp.dealerVal == 1) ? 'red' : 'black';
        document.getElementById('cardPlayer').innerText = "?";
        document.getElementById('cardPlayer').style.background = "#333";
    ...
    guess: (choice) => {
        ...
        document.getElementById('cardPlayer').style.background = "white";
        document.getElementById('cardPlayer').innerText = doubleUp.toCardStr(playerVal);
        document.getElementById('cardPlayer').style.color = (playerVal > 10 || playerVal == 1) ? 'red' : 'black';
    ...
    collect: () => {
        ...
        game.updateUI();
        setTimeout(() => { game.stage++; game.spawnEnemy(); game.updateUI(); }, 500);
    },
...
const effects = {
    ...
    addText: (text, x, y, color, isBig = false) => {
        effects.texts.push({ text: text, x: x + (Math.random()-0.5)*40, y: y, vy: -2, life: 60, color: color, size: isBig?30:20 });
    },
    explodeCoins: (amount) => {
    ...
    updateAndDraw: () => {
        for(let i=0; i<effects.texts.length; i++){
            let t = effects.texts[i]; t.y += t.vy; t.life--;
            ctx.globalAlpha = Math.max(0, t.life/20);
            ctx.fillStyle = t.color; ctx.font = `bold ${t.size}px Arial`;
            ctx.textAlign="center";
            ctx.lineWidth=4; ctx.strokeStyle="black"; ctx.strokeText(t.text, t.x, t.y);
            ctx.fillText(t.text, t.x, t.y);
            if(t.life<=0){ effects.texts.splice(i,1); i--; }
        }
        ctx.globalAlpha = 1.0;
        for(let i=0; i<effects.particles.length; i++){
            let p = effects.particles[i]; p.x += p.vx; p.y += p.vy; p.vy += p.gravity; p.life--;
            if(p.y > canvas.height-20){ p.y = canvas.height-20; p.vy *= -0.6; }
            ctx.fillStyle = "#f1c40f"; ctx.beginPath(); ctx.arc(p.x, p.y, 5, 0, Math.PI*2); ctx.fill(); ctx.stroke();
...
function loop() {
    ...
    if (game.enemy.currentHp > 0) {
        ...
        ctx.bezierCurveTo(r + 5, 60, -(r + 5), 60, -r, 0);
        ctx.fill();
        ctx.fillStyle = "white"; ctx.beginPath(); ctx.arc(-30,-20,15,0,Math.PI*2); ctx.fill();
        ctx.beginPath(); ctx.arc(30,-20,15,0,Math.PI*2); ctx.fill();
        ctx.fillStyle = "black"; ctx.beginPath(); ctx.arc(-30,-20,5,0,Math.PI*2); ctx.fill();
