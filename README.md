...
python app.py　　実行後に以下のURLにアクセスする
...
### (3) ゲーム  
シフト登録で入手したトークンをクリックで消費してモンスターを討伐するミニゲームです  
- 通常攻撃(トークン1枚消費)  
    モンスターにダメージを与えます。
...
- ステータス強化  
    トークンを消費して自身のキャラクターを強化できます。  
    - 攻撃力UP   
    - 会心UP  

- オートバトル機能    
    クリックせずにオートでモンスターを討伐することができます。　　

- 討伐報酬(High & Low)  
    敵を倒すと、獲得したトークンをさらに増やせるチャンスが発生します。  
    - 次に表示されるカードが、今のカードより高いか低いかを当てるゲームです。  
    - 予想が的中すれば獲得トークンが２倍になります。

## 3. デモ動画（GIF動画）
>ユーザーリスト
![](ユーザーリスト.gif)
ユーザーリストで他のバイトメンバーの来週のシフト、総合トークンが確認できます。  

>シフト登録
![シフト登録画面](シフト登録.gif)
シフト登録で来週のシフトが登録できます。勤務開始時間と勤務終了時間を選択してください。  

>ゲーム
![ゲーム画面](ゲーム.gif)
集めたトークンを使ってモンスターを攻撃できます。  
敵を倒すとトークン２倍のチャンス!

## 4. アピールポイント
### バイトのモチベが上がらない人におすすめです  
      
- 面倒な計算は全て自動!  
    シフトを登録するだけで給料が計算されて今月の見込みに算出されます。  
    扶養の範囲か超えないか？といった不安も、数値をチェックするだけで解消されます。  
- もう少し働こうのきっかけに  
    「あと１時間シフトを増やせばあのモンスターを倒せるかも」そんなゲーミフィケーション要素が、あなたの勤労意欲を楽しくバックアップします。  
      
- 友人と競い合える  
    ユーザーリストで他のバイトメンバーの総トークン数がチェックできます。  
    どちらがより多くのトークンを集めることができるか競い合うことができます。

- 友達と共用することでスケジュール管理が楽に
    ユーザーリストから同じサーバーに接続しているユーザのシフトが確認できるためスケジュールの共有が楽になります。
